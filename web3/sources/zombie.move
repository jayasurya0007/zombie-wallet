module zombiewallet::zombie {
    ...
    public entry fun claim_allocation(
        ...
        assert!(contains(&wallet.beneficiaries, caller), EInvalidBeneficiary);

        let data = remove(&mut wallet.beneficiaries, caller);
        ...
        while (i < len) {
            if (*vector::borrow(&wallet.beneficiary_addrs, i) == caller) {
                vector::swap_remove(&mut wallet.beneficiary_addrs, i);
                break;
            };
            i = i + 1;
        }
